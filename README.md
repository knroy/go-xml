...
| **XQuery 3.1** | 99.63% of the W3C QT3 suite (30,233 of 30,346 in scope); constructors, FLWOR, the prolog, try/catch, switch, typeswitch, windows, and both halves of `import` — `module` and `schema`. Schema import brought 416 cases into scope and 315 more passes; the 113 failures are the schema-aware tail catalogued in [todo.md](docs/todo.md) §1.5 |
...
| **XSLT 3.0** | 98.63% of the W3C XSLT suite filtered to 3.0 (11,367 of 11,525 in scope). Streaming is now measured rather than excluded, which is why the denominator grew by 2,862 cases: 118 of the 158 failures want the XTSE3430 that more of the §19.8 posture-and-sweep analysis would emit — see [Where it fails](#where-it-fails). Also measured against DocBook xslTNG and XSpec — see [Real-world stylesheets](#real-world-stylesheets) |
...
| **Tests** | 1,722 `func Test` declarations, clean under `-race` (a few subtests skip without the corpora below) |
...
|---|---|---|
| **Unit tests** (1,722 `func Test` declarations) | places where a plausible implementation is quietly wrong | anything nobody thought to write a test for |
