...
| The cast-permission table was asserted by nothing | Deleting `castPermitted`'s body left the test green: the lexical re-parse still fails, under `FORG0001`. Each row now pins `XPTY0004`, and `castable as` — a boolean, so never covered at all — is asserted too. | |
| Seven XPath tests named a W3C error code and asserted only `err != nil` | An error from any cause satisfied them. Each now reads `xdm.ErrorCode(err)`: `FODF1310`, `XPST0017` (×2), `XPTY0004` (×2), `FOCA0002` (×2), `FORG0001`. Swapping each code in production now fails the test it belongs to. | |
