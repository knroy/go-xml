...
| `fn:serialize` ignored `doctype-public`, `doctype-system` and `escape-uri-attributes` | The three had no field to write to, so a request for a doctype produced none and a URI attribute went unescaped though the parameter defaults to `yes`. `xsl:output` had honoured all three throughout. | [`PENDING`][PENDING] |
| `suppress-indentation` suppressed the whole document, and only in one spelling | It is now per-element in both the map and element forms: the named element keeps its own indentation and only its content is spared, which is what `serialize-xml-108` asserts. | [`PENDING`][PENDING] |
| Every JSON option was dropped when its name was an `xs:untypedAtomic` key | `duplicates`, `escape`, `validate` and `fallback` across `parse-json`, `json-doc` and `json-to-xml` silently did nothing. The key family is the one `xdm/maparray.go` already documents. | [`PENDING`][PENDING] |
| `xsl:function/@cache` rejected two of the three values the spec defines | `cache="full"` and `cache="partial"` were static errors, and widening the table alone would have turned that into silent loss of memoisation because the hint is read with `isYes`. | [`PENDING`][PENDING] |
| `xsl:function/@identity-sensitive` and `xsl:merge-source/@for-each-stream` were unknown attributes | Both are in the 3.0 element syntax; the merge spelling is the Recommendation's, and the withdrawn `for-each-source` stays accepted beside it. | [`PENDING`][PENDING] |
| `xsl:accumulator-rule` accepted a `priority` attribute that no specification defines | A misspelling of `phase` was swallowed in silence; it is now `XTSE0090`. | [`PENDING`][PENDING] |
| The second presentation modifier `a` and `c` raised `FOFD1340` on valid pictures | `[M1a]` was an error and `[MNna]` silently lost its name presentation and emitted a bare number. All four modifiers the grammar admits are now stripped. | [`PENDING`][PENDING] |
| `fn:default-collation()` reported codepoint whatever the static context said | It ignored its context entirely, so a stylesheet with `default-collation=` got case-blind `contains` and a contradicting `default-collation()`. The URI now travels with the collation from all three hosts. | [`PENDING`][PENDING] |
| `fn:adjust-*-to-timezone` accepted an offset that was not a whole number of minutes | `PT1M30S` was truncated to `+00:01` instead of raising `FODT0003`; the check tested integral seconds where the spec says minutes. | [`PENDING`][PENDING] |
