...
| `fn:accumulator-after` had no streamability rule, so §19.8.9.1 never fired | Absent from the operand-usage table, every call was unmodelled and suppressed its sequence constructor's verdict. Implemented the cascade; rule 8's "enclosing node" inherits into nested constructors. 11,463 → 11,466. | [`1d10349`][1d10349] |
...
| `fn:current()` in a pattern was unmodelled | §19.8.9.3 fixes it striding and motionless in a pattern, denoting the node the whole pattern matches; supplying that node's kind separates a consuming element from a motionless text node. 11,466 → 11,468. | [`1d10349`][1d10349] |
| A call on a stylesheet function inside a streamable instruction was never assessed | The §19.8.5 function table reached the body check but not the instruction analyser, so every such call was unmodelled and suppressed the whole body's verdict. Threaded it through. 11,452 → 11,454. | [`b4c4bb2`][b4c4bb2] |
...
| Five §19.8 rules missing from the streamability analysis let unstreamable stylesheets compile | `fn:outermost`, constructor functions and the `group-starting-with` pattern were unmodelled, the grouping key was assessed grounded in every clause, and a streamed document's body was only checked streamable, not grounded as §18.1 demands. 11,452 → 11,463. | [`1c43c4e`][1c43c4e] |
| A streamable rule using `current-group()` in its own grouping was refused | §19.8.8.4 widens a union of two striding operands to crawling by its own admission, so `current-group() except .` roamed and `si-group-055` was rejected though the catalog asserts output. Withheld inside the grouping only, which keeps `si-fork-116` refused. 11,443 → 11,451. | [`d0dd99d`][d0dd99d] |
...
| The body of a template rule in a streamable mode was never assessed | §19.6 makes such a rule a focus-setting container with a striding context posture, so its body is decidable alone — no fixed point over the mode's rules. 11,422 → 11,441. | [`f9c0cf5`][f9c0cf5] |
...
| `app-Demos/RexParser` | Re-measured as four distinct sites, not one: `scanToStop` and `scanExprSingleSource` bound an ExprSingle with flat counters that cannot record the nesting order of interleaved `if` and FLWOR. Four prototypes each fixed three and surfaced the fourth. | [`18a6d96`][18a6d96] |
