...
|---|---|---|
| A typed function test was answered on arity alone | `applyBuiltinSignatures` read a seventeen-entry table of its own while call binding read all 272, so `fn:concat#2` satisfied `function(xs:date, xs:date) as xs:integer` -- both take two arguments. Function items now read `specSignatures`, and `fn:concat` gets its per-arity type. | |
| Three item types rendered as a bare `item()` | `xs:numeric`, an array test and a typed function test lost their identity in `SequenceType.String()`, which is what a signature is compared through, so an identical type failed to match itself once signatures were consulted. | |
