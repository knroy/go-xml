...
| A duplicate key in an XPath map constructor used XQuery's error code under XSLT | The `map{...}` constructor is one expression with two spellings of the same failure, because the code belongs to the **host language** rather than to XPath. XQuery 3.1 §3.11.1 calls a duplicate key `XQDY0137`, which the QT3 suite requires and which remains the default; XSLT 3.0 §17.4 says of the very same `MapExpr` that "if two or more entries have the same key then a dynamic error occurs [see ERR **XTDE3365**]" — the same code `xsl:map` already raises for a duplicate among the maps it merges, so in XSLT the two ways of writing a map now agree on how they fail. The engine raised `XQDY0137` from both hosts. A new `xpath.Context.MapDuplicateCode`, set once where the XSLT runtime builds its context, joins the existing `Compat` and `RegexVersion` host knobs; leaving it unset keeps `XQDY0137`, so QT3 is untouched. Fixes `si-fork-814` and `sx-MapExpr-007`. | — |
| `xsl:source-document/@use-accumulators` was parsed and discarded | The attribute was accepted and ignored, on the recorded reasoning that an engine applying no accumulators cannot be wrong about which apply. That reasoning had **decayed**: accumulators are implemented, and §18.2.2 makes the attribute the set of accumulators *applicable* to the document the instruction reads, which `XTDE3362` makes it a dynamic error to read outside. Nothing in that rule is conditional on streaming — as the case's own description states, "Use-accumulators applies even when not streaming" — so a stylesheet naming one accumulator and reading another ran quietly to completion. The identical set and enforcement already served `xsl:merge-source/@use-accumulators`, so the fix parses with the same `parseUseAccumulators` and registers the result in `rt.treeAccums` exactly as `merge.go` does; an absent attribute still leaves the tree unrestricted. Fixes `non-stream-201`. | — |
| **The XSLT instruction rules of §19.8.4, on the streamability lattice** | §19.8.4 gives each of the 43 XSLT instructions its operand roles and their usages, and for most of them that is the whole rule: the general rules of §19.8.1 do the rest, so `xsl:value-of`, `xsl:element`, `xsl:comment`, `xsl:attribute`, `xsl:message`, `xsl:assert`, `xsl:result-document`, `xsl:if`, `xsl:map-entry`, `xsl:perform-sort` and a dozen more are a usage table handed to the existing `combine`. `xsl:choose` and `xsl:try` add the **choice operand groups** that let each branch consume the stream, since only one is evaluated. `xsl:for-each`, `xsl:iterate` and `xsl:for-each-group` state their own ordered cascades and each is written out clause by clause in the spec's order, the order being load-bearing — they are "the first of the following that applies". `xsl:map` implements the implicit fork over `xsl:map-entry` children; `xsl:merge` the flat grounded-and-motionless condition of §19.8.4.25; `xsl:copy` treats its select as **focus-setting** rather than as a competing operand, without which a nested copy chain counts each level twice and `si-copy-028` (four deep) is wrongly refused. Alongside them, **§19.8.6 attribute sets** — both halves: a reference is judged on the set's *declared* streamability, because another package may override it, and a set declaring `streamable="yes"` is separately checked for honesty — and **§19.8.7 value templates**, whose operands are the expressions inside the braces. All of it lives in a new `xslt/streaminstructions.go`; the only edits to existing files are the dispatch and attribute-set collection in `streamcheck.go` and one `known`-propagation fix in `streamability.go`. That fix is the subtle one: §19.8.8.7's scanning-expression test answered "not a scanning expression" both when a path really is not one and when a predicate held something unmodelled, and reporting the second as a fact about the stylesheet refused `si-group-051`, which filters on `current-group()`. The same discipline governs the cascades — a clause that rejects an instruction without consulting its body must still establish the body was modelled, or a gap in the analysis is reported as a defect in the stylesheet. Where the Last Call draft and the test set disagree about `xsl:fork` the test set wins: the draft rejects any non-grounded branch, but `si-fork-A`'s `f-006` pairs a grounded branch with a striding one and is expected to run, while the spec's own two-`copy-of` example is expected to be streamable with both branches consuming, so the implemented rule counts *non-grounded* branches and rejects more than one. XSLT 3.0 **11,371 / 154 → 11,381 / 144**, diffed by name in both directions: ten fixed (`si-lre-901`, `si-lre-902`, `si-copy-901`, `si-copy-902`, `si-element-901`, `si-element-902`, `si-for-each-806`, `si-for-each-905`, `si-fork-901`, `si-iterate-905`) and **nothing newly failing**. XSLT 2.0 unmoved at 6,193 / 8, byte-identical by name. Across the whole 456-stylesheet streaming corpus the analysis rejects 29 `xsl:source-document` bodies and every one of the 29 is a case whose catalog entry asks for `XTSE3430`. Ten instructions remain unimplemented — `xsl:apply-templates`, `xsl:call-template`, `xsl:apply-imports`, `xsl:next-match`, `xsl:next-iteration`, `xsl:break`, `xsl:analyze-string`, `xsl:number`, `xsl:evaluate`, `xsl:stream` — six of them blocked on §19.4 type-determined usage rather than on the instruction rule itself; each is reported unmodelled rather than guessed at. | — |
| **Streamable stylesheet functions — XSLT 3.0 §19.8.5** | §19.8.5 gives every `xsl:function` a **streamability category** — `unclassified` (the default), `absorbing`, `inspection`, `filter`, `shallow-descent`, `deep-descent`, `ascent` — and the category does two jobs: it constrains the function **body** (each category names the posture and sweep the result must have) and it determines the posture and sweep of a **call**, which is what lets the analysis continue through a call site instead of abandoning the enclosing construct there. All seven categories are now implemented in `xslt/streamfunctions.go`: the body constraint and call rules of each, the rule that a zero-arity function may only be `unclassified` and that calls on it are grounded and motionless, and §19.8.5's requirement that an unrecognized implementation-defined category be analysed as `unclassified`. Two §19.8.8 rules came with it because §19.8.5 is unreachable without them: **§19.8.8.11**'s streaming-parameter posture table (a reference to the first parameter of a declared-streamable function is not grounded — its posture comes from the category and from whether the reference is *singular*), and **§19.8.8.6**'s simple map operator `!`, which was previously unmodelled and is the dominant call-site shape in the suite (`path ! f:g(., 'x')`). The `known` guard is untouched and still load-bearing: a construct the analysis does not model remains "no opinion" rather than a rejection, and a **directly recursive** declared-streamable function is deliberately reported as unmodelled, because assessing it in one pass rather than to the fixed-point §19.8.5.2's third example describes gives a wrong verdict. Measured: XSLT 3.0 **11,371 → 11,385** passing, 154 → **140** failing, the by-name diff moving exactly 14 cases in one direction and **none** in the other — `su-absorbing-903`/`-904`, `su-ascent-901`/`-903`/`-904`, `su-filter-901`/`-902`/`-903`/`-904`, `su-inspection-904`, `su-shallow-descent-903`/`-904`, and `sx-fcall-026`/`-027` as a side effect of the two §19.8.8 rules. XSLT 2.0 unmoved at 6,193 / 8 with a byte-identical failing list. Compiling all **8,730** stylesheets in the corpus raises `XTSE3430` on 34, every one of which either wants it or already got it before this change — **no stylesheet that should compile is refused**. Three spurious refusals were found by that whole-corpus check and fixed rather than shipped: a declared return type of `node()*` was given the *navigation* type-determined usage, which made the result of every node-returning function roaming and wrongly rejected `su-filter-101a`, `-102` and `su-filter-A`; a recursive ascent function came out striding where §19.8.5.7 wants climbing (`su-ascent-A`); and a path rooted at the streaming parameter, `$node//section`, was not recognised as a scanning expression where the identical `.//section` is (`function-5016`, `function-5006a`). What §19.8.5 does **not** close is named in [docs/conformance-gaps.md](docs/conformance-gaps.md): the `su-*` families keep 13 failures wanting rules from elsewhere — `for` expressions, the `fn:deep-equal` and `fn:path` operand usages, and the recursive fixed-point. | — |
| **XSLT 3.0 §18.2.8: a streamable accumulator is held to its five conditions** | §18.2.8 makes an `xsl:accumulator` with `streamable="yes"` *guaranteed-streamable* only when its `applies-to` pattern and every rule's `match` pattern are **motionless** (§19.8.10), and its `initial-value` and every rule's `select` are **grounded and motionless** (§19.5, §19.7). `xslt/streamaccumulators.go` checks all five on top of the existing lattice, raising `XTSE3430` when one fails. §19.8.10 is implemented as its top-level-predicate rule — each predicate's expression must be motionless *and* non-positional — with the classification abandoned for a `RootedPath`, which production [6] of the pattern grammar defines as a pattern *starting* with a variable reference or `doc`/`id`/`element-with-id`/`key`/`root`, **not** a leading `/`: §19.8.10's own list calls `/`, `/*`, `p/q` and `//p/text()[. = 'Introduction']` motionless. Two focus facts the rule turns on: an accumulator rule's `select` is evaluated with the context item being the node its own `match` selected, so `string()` on a `text()` rule is motionless where the same expression on an element is consuming; and §18.2.1 converts the result to the declared `as` type by the function conversion rules, so an atomic `as` **atomizes and thereby grounds** a `select` that would otherwise return a streamed node — which is what makes `as="xs:double"` with `select="if (@amount < $value) then @amount else $value"` legal. The `known` discipline is preserved exactly: an unmodelled pattern or expression is abandoned in silence rather than rejected. Fixes `accumulator-019s`, `-029s`, `-030s`, `-076`. XSLT 3.0 11,371 → **11,375** / 11,525; XSLT 2.0 unmoved at 6,193 / 8; **0 spurious `XTSE3430` across either corpus**. | — |
| **`fn:string()` and `fn:string(.)` classify identically** | §19.8.1 downgrades an absorption to motionless when the operand cannot have children, which is how a predicate on `@code` or `text()` stays motionless. The zero-arity form of a context-item-defaulting built-in — `string()`, `data()`, `name()` and the rest — built its implicit `.` operand with `allowsChildren` hardcoded to `true` in `xslt/streamability.go`, so `string()` came out **consuming** on a text-node context item where the equivalent `string(.)` came out motionless. The implicit argument is `.`, so it now carries the context item's own answer. One line; it is what `accumulator-077s` turned on. | — |
| A `schema-element(E)` test matched members that could never validate a node | `SubstitutionGroupMembers` handed the type test the whole of `ElementDecl.Substitutable()`, which answers a different question. That set is a **content-model** answer — which declarations may appear where a particle names the head — and a validation episode checks each candidate against the instance in front of it, so it rightly keeps members a type test must not. A `schema-element(E)` test has no instance to check: it asks whether an already-validated node *could* have been validated against `E` or something substitutable for it, so a member that could never produce such a node has to be dropped up front. Two can't. An **abstract** member, because §3.3.6 forbids an abstract declaration from validating any element itself — only its non-abstract substitutes can — yet it must stay in `Substitutable()`, since a content model naming the head still admits what it leads to, which is exactly why the filter is a separate method rather than a narrowing of the closure. And a member that forbids nilling under a head that **permits** it, because XPath 3.1 §2.5.5.3 makes the nilled property part of the element test: the head's test admits a nilled node and a non-nillable member can never yield one. That clause runs one way only — a nillable member under a non-nillable head still substitutes, since it only ever yields nodes the head's test already admits. `ElementDecl.SchemaElementMembers` in `xsd/component.go` applies both, and the three implementations of the `xpath.SchemaTypes` seam (`xquery`, `xslt`, the XSLT judge) share it, so the rule cannot drift between hosts. Fixes `substitution-020`, `-021`, `-024` and `-025`; `-022` and `-023`, whose member is neither abstract nor non-nillable, keep passing. | — |
| A function test compared schema type names written in two different alphabets | `local:f#1 instance of function(s:myUnionType1) as xs:boolean` answered false for a function *declared* `s:myUnionType1` — the same type named twice. Function subtyping compares type **spellings**, and the two sides reached the comparison by different routes: a sequence type resolves a schema type at parse time into the key the data model records annotations under, Clark notation `{uri}local`, while `declaredSignature` recorded the raw source text with whatever prefix the query author bound. Comparing `{…}myUnionType1` against `s:myUnionType1` is comparing a name against a different spelling of itself, so a schema type could not match even *itself*, and every relation built on top of that was unreachable. `typeSource` now renders the compiled type rather than the source text, putting both sides in the alphabet the resolution actually happened in; for a built-in the two forms coincide, so nothing that passed changes. Fixes `instanceof135` on its own and is the precondition for the row below. | — |
| Schema-defined union and restriction types had no subtype relation | With both sides finally spelled alike, the relation itself was still missing: `atomicSubsumes` knew only a hardcoded table of built-ins, so a schema type subsumed nothing but itself and XPath 3.1 §2.5.6.2's Judgement 1 went unanswered over the derives-from relation of §2.5.5. Three clauses now answer it, from the registries `xsd` already fills as it loads a schema — no schema is carried into the evaluator and no new interface was needed. **Restriction**: walk the sub-type's base chain toward the super-type. **Membership**: §2.5.5 makes union membership a clause of derives-from in its own right, so every member of a union is a subtype of it. **Subset**: one union is a subtype of another exactly when every one of its members lands inside the other — the *smaller* union being the subtype, as the suite notes on `instanceof139` ("there is a subtype relationship between union(A,B,C) and union(A,B)"). The two mixed shapes fall out of the same loop: a union all of whose members derive from one built-in is a subtype of it, and a built-in is a subtype of a union having a supertype of it as a member. One wrinkle is load-bearing: the registries key built-ins by their **bare local name**, since `AnnotationName` drops the XSD namespace, while a signature spells the same type `xs:date` — the walk translates between the two, and without it the members of every union compared unequal to the built-ins they name. The walk is depth-bounded rather than cycle-tracked, `xsd` having already rejected a cyclic chain at load, so a malformed registry answers wrongly rather than hanging. Fixes `instanceof136` through `141`, taking `prod-InstanceofExpr` to 308 / 308. | — |
| A function result converted to an imported schema type failed the type it was converted to | XPath 3.1 §3.1.5 atomises a returned node and casts the resulting `xs:untypedAtomic` to the declared type — the rule that makes `declare function f() as xs:integer { <e>1</e> }` return `1`. Both halves ran correctly for a schema-defined type and the value was still refused: `castOne` reached `xpath.CastToDerived`, which stamps the derived name through `WithDerived` only for the **built-in** facet tables it knows, so a restriction of `xs:date` came back as a bare `xs:date` carrying no annotation. Matching a `SchemaType` asks for exactly that annotation, so the freshly converted value failed `MatchesItem` and the conversion reported `XPTY0004` on a value it had just produced correctly. The same defect had already been fixed on the `cast as` path, whose comment describes it precisely — "without recording the annotation the cast produced a bare primitive, so `foo:testType(2000)` did not match a declared type of `foo:testType`" — and the function conversion rules simply never got the same treatment. `castOne` now applies the schema type's own facets through `SchemaValueValid` and stamps the identity with `WithDerived`, which is both halves of what the declared type means: the schema's constraint decides the value, and the value is an instance of the schema's type rather than of the primitive it restricts. Fixes `qischema040` and `-040a`. | — |
| Lax validation skipped an undeclared element that named its own type | XSD 1.0 §3.3.4 clause 1.2.1.2 resolves an `xsi:type` attribute and assesses the element against the type it names **whether or not a declaration exists**, and lax assessment takes that path as readily as strict does — `CanAssessStrictly`, immediately below the defect, already stated the same rule for the strict side. `ValidateElementLax` tested only for a global declaration and returned `nil` when it found none, so `validate lax { <z:person xsi:type="xs:NCName">abc 123</z:person> }` returned the element unassessed where `"abc 123"`, holding a space no `xs:NCName` may, owes `XQDY0027` by XQuery §3.21. The skip now also requires the absence of `xsi:type`; an element with neither a declaration nor an `xsi:type` is still skipped, which is what lax means and what the accompanying test pins. `Validate` already resolved and honoured `xsi:type`, so the fix is the guard rather than any new assessment machinery. Fixes `qischema90621-err`. XSD 1.0 agreement unmoved at 39,358 and XSD 1.1 at 41,545. | — |
| A path step was accepted after a `validate` expression | `[102] ValidateExpr` is an `ExprSingle` form and is **not** among the `[128] PrimaryExpr` alternatives a `StepExpr` reaches through `[121] PostfixExpr`, so no step and no predicate may follow one — unlike `[136] OrderedExpr` and `[137] UnorderedExpr`, which *are* primaries and which `PathExpr-21`'s `/ordered{bid}` relies on. The operand-substitution path lifted the validate expression into a synthetic call and handed the rewritten source to `xpath`; a call **is** a primary, so `validate { … }/*` became `f()/*` and parsed cleanly as the path the grammar forbids, silently returning the validated element's children where `XPST0003` is owed. Substitution is now abandoned when a `/` or `[` follows a validate expression, which leaves the delimiter for the caller to report as the same `XPST0003` that `validate { … } bogusword` already produced; `withTrailingPath` refuses the identical shape on the path it owns. Parenthesising still works, because `[133] ParenthesizedExpr` is a primary — the contrast that makes this a rule about the grammar rather than about the value. Fixes `qischema90007`. | — |
| An `<environment>` schema was registered as source text, so its own `xs:import` was never followed | `caseSchemas` read the document the catalog named and handed it over as `Source`, with no `BaseURI` and no resolver, on the reasoning that nothing would follow a relative reference anyway. But a schema document may `xs:import` a second one beside it, and `qischema032.xsd` does exactly that — the `res:resource` **complexType** its own `xs:restriction` bases on lives in `qischema032a.xsd`. Source text alone left that type undefined, so the import failed with `XQST0059: … src-resolve: type "res:resource" names no type definition` and the engine never got as far as the case. `hintedSchemas` beside it had already solved this and says so in its own comment — "qischema083's first document `xs:imports` its second, which only an assembly with a base can follow" — so the fix is the one already in the file: assemble with `xsd.LoadFiles` and register the `Components`. The read stays the *harness's*: `LoadFiles` confines its resolver to the directory of the file the catalog named, and `Options.SchemaResolver` is still nil, so an `at` hint in a query's own text still opens nothing. Fixes `qischema032`, `-032a`, `-032b`, `-032c`. | — |
| `prod-ContextItemDecl/contextDecl-052` | A W3C fixture defect, re-derived from the files: `libmodule-3.xq` declares target namespace `…/libmodule1` while the catalog registers it under `…/libmodule3`. §4.11 makes the target namespace the module's identity, so §4.12 owes `XQST0059` — returned right after `parseModuleDecl` and before the prolog that holds the `declare context item`, which puts the wanted `XQST0113` behind a correct check. `modules-bad-ns` pins the same rule from the other side. | — |
| `app-Demos/RexParser` | The failing construct is now named rather than guessed at: a **multi-clause FLWOR in a conditional branch**, in `p:transition`. The reported "offset 0" is an artefact of `parseNestedExpr` resetting the cursor before it refuses. Two lexical scans mis-cut the shape, and fixing the second (`scanToStop`) cost three regressions — `cbcl-boolean-001`, `xqhof1`, `cbcl-group-by-key-003`, each truncated mid-clause — for zero gains, so both fixes were reverted. Recorded in `docs/conformance-gaps.md` with the measurements. | — |
| `misc-CombinedErrorCodes/FOTY0012` | Confirmed to need an `xdm` change, not a `data()` fix: after `validate strict` the node's `TypeAnnotation` is `anyType`, so the anonymous complex type the error depends on is never recorded, and the typed-value walk has no failure mode at all. `FOTY0012` appears nowhere in the tree. | — |
| `K2-sequenceExprTypeswitch-5` | Re-derived: `parser.vars` holds only prolog declarations and `typeswitch.go` never consults it, so there is no in-scope set a per-branch `XPST0008` check could read. The recorded trade against `K2-ForExprWithout-8` still holds. | — |
| A library module's prolog was parsed and then thrown away | `libModule` carried a module's variables, functions and decimal formats and nothing else, so anything else its prolog declared could not bind on the module that imported it. Two rules were lost that way. §4.16 lets a library module constrain the context item's **type** without supplying its value — a value there is `XQST0113`, but a type is the module saying what it needs — and that constraint binds on whatever item the importing query is evaluated with; the declaration now survives compilation and `Query.checkImportedContextItemTypes` applies every imported type alongside the main module's own. An absent item is deliberately not this check's complaint: a module that declares a type says what the item must be *if* there is one, and whether there must be one at all is the main module's `XPDY0002`. The same discarding hid §2.2.4's `XQST0108`: an `output:` declaration in a library module is a static error outright, because serialization describes a query's **result** and a library module has none, so the declaration could never take effect — and two imported modules could each set a different `output:method` with nothing to arbitrate between them. It is now refused where it is written, while the identical declaration in a main module stays legal and still takes effect. Fixes `contextDecl-050`, `contextDecl-051` and `Serialization-003`. | — |
| `XQST0058` was reported as `XQST0059` because the schema was fetched first | §4.11 forbids importing one schema target namespace twice in a prolog, and that is a fault of the prolog's **text**: two imports of one namespace contribute the same components whether or not either schema can be found. The check existed, at the end of `parseSchemaImport`, and could never be reached — each `import schema` is *loaded* where it is read, so an unresolvable **first** import aborted the prolog with `XQST0059` before the second import was parsed at all. Both suite cases import a namespace that cannot be resolved, which is exactly the shape that hid the rule. §2.2.4 makes a static error one that is raised regardless of the input, so the query's own fault outranks the environment's: `parseProlog` now runs a text-only pre-scan over the whole prolog before any of it is followed. The scan is deliberately conservative — it walks the leading run of declarations, stops at anything it does not recognise, and records a namespace only where it can read one unambiguously — because a missed import costs only this error, while a guessed one would cost a wrong refusal. Fixes `misc-CombinedErrorCodes/XQST0058` and `prod-SchemaImport/schema-import-2`. | — |
| `current-merge-group()` with an unknown source name was unreachable behind an earlier error | XSLT 3.0 §15.6.1 makes it `XTDE3490` if the `$source` argument "does not match the `name` attribute of any `xsl:merge-source` element for the current merge operation", and ends "The error may be reported statically if it can be detected statically". The runtime check was already correct and already raised it — but only from inside `xsl:merge-action`, which a stylesheet whose input the runtime rejects first never reaches: `merge-077` names a source that does not exist *and* has input that trips `XTDE2220`, so the required error could not be reported at all. `compileMerge` now takes the same static licence `checkMergeKeyCompatibility` takes for `XTDE2210` two checks earlier, scanning the action's `@select` expressions for a `current-merge-group()` call whose argument is a string literal and reporting `XTDE3490` when it names no declared source. Only literals are decided: a computed argument has no static value and stays with the runtime check, which remains the normative report. A nested `xsl:merge` is not descended into, because §15.6.1 scopes the current merge group to the innermost containing `xsl:merge-action`, so `'inner'` is that merge's name to declare and not this one's to refuse. The literal reaches `xslt` through a new `StringArgs` field on `xpath.StaticCall`, populated by the `walkCalls` traversal that already existed — additive, and nil for the overwhelming majority of calls that carry no string literal. Fixes `merge-077`: 11,370 → 11,371 passing, 155 → 154 failing, with `merge-077` the only name to move in either direction. XSLT 2.0 unmoved at 6,193 / 8; XPath 100% at 2.0, 3.0 and 3.1 and XQuery unmoved at 30,280 / 66. | — |
| A cast target ate the operator after it | XPath 3.1 gives `cast as` and `castable as` a **SingleType** — `[77] SingleType ::= SimpleTypeName "?"?` — not a SequenceType, so `?` is the only occurrence indicator it may carry. The parser used `parseSequenceType` for both, which consumes `*` and `+` greedily, so in `15 cast as t:sizeType + 15 cast as t:floatBased` the **additive `+` was swallowed as an occurrence indicator** and the expression died with `XPST0003: a cast target may not have an occurrence indicator "+"` — an error about an operator the query never wrote as one. `checkCastTarget` was rejecting after the fact what the parser should never have eaten. A `Parser.singleType` flag now narrows the accepted indicators to `?` for the duration of a cast target, honoured at both the shared `occurrence:` label and `finishOccurrence`, including the wildcard-spelled `*` fallback. A target written with a *genuine* `*` or `+` is still `XPST0003`, as `K-SeqExprCast-1` and `-2` require: the indicator is no longer part of the type, so it is left behind as an operator with no right operand and the expression fails to parse — the diagnosis moves, the error code does not. Every other type position keeps all three indicators, which the XPath lanes confirm unmoved at 100% on all three versions. Fixes `op-numeric-add-13`, `-14`, `-15` and `-16`, taking `op-numeric-add` to 144 / 144. | — |
| A URI literal was trimmed where the spec collapses it | A `URILiteral` has type `xs:anyURI`, whose whitespace facet is **`collapse`**, not a trim: leading and trailing whitespace is removed *and* every internal run of whitespace becomes a single space. The prolog applied `strings.TrimSpace`, which is only the first half. That satisfied `module-URIs-1` and `-2`, which pad the ends and were the only cases exercising the rule — and left `module-URIs-3` failing, where the import writes `"Test    Modules"` and the module is registered as `"Test Modules"`: the two name one module and the import could not find it. A new `collapseURI` applies the facet at both namespace sites of a module import and a schema import, and at the **module declaration's own** target namespace, so a declaration and an import written with different whitespace still meet. Fixes `module-URIs-3`. | — |
| A library module's context item declaration could carry a value | §4.16: "A context item declaration in a library module must not specify a value, and must not be declared external with a default value." A library module does not own the context item — the main module supplies it and the library may only constrain its **type** — so an initialiser there is a value nothing could ever use. The parser accepted it silently, and the query then failed at evaluation with `XPDY0002` for an unbound `.`: a dynamic error, and the wrong one, for a static defect. The check keys on the **initialiser** rather than on the `external` keyword, because the rule is broken both ways round — `contextDecl-048`'s module writes `external := 17` and the type-only `declare context item as xs:integer external` stays legal. Fixes `contextDecl-048`. | — |
| A typeswitch case accepted a type that is not an item type | §3.14.2 gives a `CaseClause` a SequenceType, so its ItemType obeys §2.5.4: an `AtomicOrUnionType` there must be a **generalized atomic type** — an atomic type, or a *pure* union whose members are all atomic. A list type, or a union derived by restriction or holding a list member, names a type but **not** an item type, and using one is `XPST0051`. The gate already existed and was already applied at `instance of`, `treat as` and function signatures; `parseTypeUntil` called `xpath.ParseSequenceType` raw and so skipped it. The impure type then simply failed to match and the `default` branch answered — **a static error turned into a wrong value**, which is the worse of the two failure modes. `xpath.CheckItemTypePurity` exports the existing check for the host language rather than copying it. The check is on *purity*, not on being a union, so `typeswitch-116`'s pure union of a union keeps passing. Fixes `typeswitch-113`, `-114` and `-115`. | — |
| An inline function converted its arguments by different rules than a declared one | Two inline-function implementations existed and disagreed, and which one ran was decided by the **body**: if the body needed no XQuery-only syntax, the whole construct — *signature included* — went to xpath's `convertForParam`, which is schema-blind. So the same declared type behaved differently depending on how the function was spelled. §3.1.5 says an `xs:untypedAtomic` supplied for a pure union is **cast** to a member type, and that a **namespace-sensitive** target refuses the conversion outright with `XPTY0117` — the prefix bindings needed to resolve the string are not in scope at the call site, so nothing the caller could write would succeed. This package implements both rules correctly; the fork simply bypassed them, and both cases arrived as a flat `XPTY0004`. `parseInlineFunc` now keeps ownership whenever a declared parameter or return type is a schema simple type, so only the *conversion* moves — the body still goes to xpath, and the union logic is not copied into a third place. Fixes `FunctionCall-031` and `-041`. | — |
| Numeric promotion was applied to a union target | §3.1.5 offers a **typed** value exactly two conversions: the `xs:untypedAtomic` cast and numeric/`xs:anyURI` promotion. Promotion is defined against a single target type, **not** a union, so a typed value matching no member has no conversion left and is `XPTY0004`. `castOne` called `CastToUnion` for *any* atomic, so `xs:decimal` 12.3 was cast into a union of `xs:integer`/`xs:float` and passed — exactly the promotion `FunctionCall-030` exists to forbid. Restricting the union cast to `xs:untypedAtomic` matches the rule the function's own doc comment already stated. This was a **pre-existing defect in the declared-function path** that no suite case reached: `FunctionCall-030` is written as an inline function, and its correct answer came from the schema-blind converter above — so fixing the routing exposed it, and a unit test now pins both spellings against the spec rather than against each other. | — |
| An `<assert>` expression had no context item | The catalog evaluates an `<assert>` with the result of the test as **both** `$result` and the **context item**. The harness bound only `$result`. Almost every case in the suite writes the `$result` spelling, which is why the omission survived: the three that write an absolute path — `modules-31`, `-32`, `-33`, asserting `/result/impl = "hello"` — reported `XPDY0002: no context item` **from the assertion**, and the symptom read like a module-import defect. Running the three queries directly returned exactly the documents they assert (`"hello"`, `"helloworld"`, `"42"`), which settled it: the engine had resolved functions across two library modules sharing one target namespace correctly all along, and only the measurement was wrong. The result is made the context item when it is a single item; a non-singleton leaves the context empty, which is what an absolute path against a non-node result should do anyway. Fixes `modules-31`, `-32` and `-33` — none of which was an engine defect. | — |
