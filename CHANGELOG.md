...
| The base-cycle check walked the chain once per type | `checkTypeBaseCycles` was the residual O(N²) after the facet merge: 10,000 chained types cost 4.3 s, a 10,001-type ring 7.8 s. Proven-acyclic and on-cycle types are remembered, so both now take under 0.2 s. |  |
