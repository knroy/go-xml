...
| `app-Demos/RexParser` | The failing construct is a multi-clause FLWOR in a conditional branch; "offset 0" is `parseNestedExpr` resetting the cursor. The `scanToStop` fix cost three regressions for zero gains and was reverted. | — |
| `misc-CombinedErrorCodes/FOTY0012` | Needs an `xdm` change, not a `data()` fix: after `validate strict` the annotation is `anyType`, so the anonymous complex type the error depends on is never recorded. | — |
| `K2-sequenceExprTypeswitch-5` | `parser.vars` holds only prolog declarations and `typeswitch.go` never consults it, so there is no in-scope set a per-branch `XPST0008` check could read. | — |
