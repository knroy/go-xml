...
// Package xmlfork implements a simple XML 1.0 parser that
...
func isName(s []byte) bool {
	...
	if !xmlname.IsNameStartRune(c) { // go-xml: XML 1.0 5e [4] NameStartChar
		return false
	}
	for n < len(s) {
		s = s[n:]
		...
		if !xmlname.IsNameRune(c) { // go-xml: XML 1.0 5e [4a] NameChar
			return false
		}
	}
	return true
...
// The XML 1.0 First Edition `first` and `second` RangeTables that stood here
// upstream are deleted. This fork resolves NameStartChar and NameChar through
// internal/xmlname, which transcribes the Fifth Edition productions. See
// NOTICE in this directory for the provenance and the full diff.
