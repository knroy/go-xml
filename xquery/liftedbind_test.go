...
func TestLiftedOperandReachesEveryEvaluator(t *testing.T) {
	...
	compile := func(expr string) *compiledExpr {
		t.Helper()
		c, err := xpath.CompileWith(expr, xpath.CompileOptions{Namespaces: sc, Version: 31, XQuery: true})
		if err != nil {
...
func TestInspectDoesNotEvaluate(t *testing.T) {
	sc := newStaticContext()
	c, err := xpath.CompileWith("fn:count((1,2))", xpath.CompileOptions{Namespaces: sc, Version: 31, XQuery: true})
	if err != nil {
