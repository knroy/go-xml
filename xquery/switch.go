...
func (p *parser) parseSwitch() (node, bool, error) {
	...
	// The default branch is the last thing in the switch, so no keyword of
	// this construct follows it -- but a clause of an enclosing FLWOR may, and
	// its keyword ends this ExprSingle just as "case" ended the ones before
	// it. See enclosingClauseStops, and typeswitch.go, whose default branch is
	// the same shape.
