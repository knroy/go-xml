...
| **xquery** | QT3 — XQuery 3.1 | 30,346 | 30,266 | 99.74% | **80** | 0 | 0 | **80** | 99.74% |
...
**XQuery 3.1: 30,266 / 30,346 = 99.74%** — what passes now. The denominator
...
cases pass and 80 fail, which is the whole of the increase in the failure
count. The passing count rose by more than 311 — 29,918 → 30,266 — because four
...
typed-input and annotation gaps directly. `prod-CastableExpr` is down to
**0** — the six that wanted a pattern facet matched against a value's
*canonical* representation rather than the result of `string()` are fixed, and
so are the two impure-union cases beside them. That rule turned out **not** to
live inside `xsd`, as this paragraph once predicted: validation is handed a
lexical form, and the defect was in which form the *cast* chose to hand it.
See [todo.md](todo.md) §1.5 *The canonical form and the list member, closed*.
No set outside that catalogue gained a failure.
...
and blind to the next. The engine defect was real and is fixed. The lesson is
not that the first answer was right but that a case can fail for several
independent reasons at once, and fixing the visible one is what reveals whether
there was another.

The case then proved the point a fourth time, against this very paragraph. The
sentence that stood here — "the case still fails on the indent width alone" —
was itself a layer's worth of blindness: setting the indent to 3 spaces gains
nothing (6193/8 either way; the offset simply moves from 46 to 134), because
the width was never what fails. Underneath it sits a second difference, the
newline we write before `<style>` and Saxon does not. So the paragraph
describing the trap had fallen into it, which is the strongest argument for
re-measuring a verdict instead of re-reading it.
...
|---|---|---|---|
| `format-number-070` | `XTDE0040: no template named "main"` | **Not implementable** | Suite defect. The catalog invokes `<initial-template name="main"/>`; the stylesheet contains exactly one template, `match="root"`, and zero occurrences of `name="main"` (verified by grep). The spec: XTDE0040 is raised when the invocation "specifies a template name that does not match the expanded-QName of a named template defined in the stylesheet", and §2.9 makes it a **non-recoverable** dynamic error — mandatory, not discretionary. Passing means violating the spec. No third-party result settles it either way: the case was created in 2020 and does not appear in `Saxon_9.8.xml` at all. |
...
| `docbook-001` | `XTMM9000: Can't make chunks with go-xml's processor.` | **Not implementable** | The vendored DocBook XSL 1.79.1 writes its chunks through vendor extension elements, and `chunker.xsl` offers a choice of **three**: `element-available('saxon:output')`, `element-available('exsl:document')` and `element-available('redirect:write')`, each tested at two sites. All three are outside the XSLT specification — the `saxon:output` branch even sets `saxon:character-representation` — so passing means implementing Saxon's or Xalan's proprietary output elements, i.e. impersonating another vendor. When no branch matches, an `xsl:otherwise` reaches `<xsl:message terminate="yes">` that concatenates the literal text `Can't make chunks with `, `system-property('xsl:vendor')` and `'s processor.` **The error string is the stylesheet's own words about us, not a diagnostic of ours** — which is why the symptom moved (this row once read `XTDE1450: exsl:document is not available`) while the verdict did not. |
...
| `import-schema-137` | `XTTE1512` where `XTTE1510` is wanted | **Not implementable** | Both errors are genuinely present: `z:familyname` is absent from `schema061.xsd` (only `surname` is declared) so XTTE1512 is right for that node, while the enclosing `z:person` is invalid against `personType` so XTTE1510 is right for that one. §2.9 settles the choice by declining to: "**It is implementation-dependent which of the several errors is signaled.** This applies both to static errors and to dynamic errors." Either answer conforms; the suite tests one processor's order — `Saxon_9.8.xml` records `result="pass"`, which is what a catalog written against one implementation's detection order looks like from the other side. |
| `validation-0201` | Serialisation differs at offset 46 | **Implementation-defined** | Same case as the 3.0 entry below. The engine defect that stood behind the indentation is **fixed**: a union's selected member type was dropped whenever the tree was copied, so `xsl:strip-space` untyped the document and `data(.) instance of StandardDate` went false. Strip *all* whitespace from both sides and they are now byte-identical, so nothing but whitespace placement remains. **The row's previous claim — "what remains is the indent width" — was wrong, and the case made this document's own lesson one more time.** Setting the indent to 3 spaces was measured across the whole lane: it gained nothing, 6193/8 either way, and merely moved the reported offset from 46 to 134. The width was never the blocker. The actual difference is a *second*, independent one: inside `<head>` we write a newline before `<style>` and Saxon does not, because Saxon declines to indent before an element whose own content is significant text. Ours is the parent's decision — `hasTextChild(<head>)` is false, so `indentChildren` stays on — and it is permitted: Serialization 3.1 §5 licenses added whitespace "only where the effect is not significant", constraining where a serialiser *may not* indent rather than where it must, while XSLT 2.0 §20 makes "the amount of indentation to be used when `indent="yes"` is specified" implementation-defined outright. `<style>` is deliberately absent from `htmlPreserveWhitespaceElement`, whose set is HTML 4.01 §9.3.4's and HTML5's: `pre`, `listing`, `plaintext`, `textarea`, `xmp`. Pinned by `TestXHTMLIndentsBeforeStyleWithTextContent`. See the 3.0 row. |
...
business. `validation-0201`'s remaining difference is whitespace placement and
nothing else — strip all whitespace from both sides and they are byte-identical
— but it is **not** the indent width, as this paragraph used to say. Three-space
indentation was measured and gained nothing; what remains is the newline we
write before a `<style>` whose content is significant text, where Saxon writes
none. Both are permitted. The engine defect that used to stand behind all of
this — a union's selected member lost on every tree copy — is fixed.
